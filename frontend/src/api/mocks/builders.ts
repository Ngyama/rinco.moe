...
import type {
  ...
  HotGameDetail,
  HotGamesResponse,
  BangumiTrendResponse,
  ControversyResponse,
  ExtremeBarsResponse,
  StaffPersonsResponse,
...
export function buildGlobalDualTop(limit = 25): DualTopResponse {
  ...
  const toItem = (w: MockWork, rank: number, score: number): GlobalScoreItem => ({
    rank,
  ...
  const combined = [...pool]
    ...
    .slice(0, 10)
    .map((row, i): CombinedRankItem => ({ rank: i + 1, ...row }));

  return { limit: take, bangumi, vndb, egs, combinedTop10: combined };
}

export function buildTrend(minVotes = 50): BangumiTrendResponse {
  const points = MOCK_WORKS.filter((w) => sumVotes(w.ratingCounts) >= minVotes || w.votes >= minVotes).map(
    (w) => ({
      year: w.year,
...
export function buildHotList(year: number, limit: number, sort: string): HotGamesResponse {
  ...
  if (games.length < want) {
    const rest = MOCK_WORKS.map(toHotItem).filter(
      (g) => !games.some((x) => x.bangumiSubjectId === g.bangumiSubjectId)
    );
    games = [...games, ...rest];
  }
...
export function buildHotDetail(id: number): HotGameDetail | null {
...
function controversyItem(w: MockWork) {
...
export function buildControversy(yearFrom: number, yearTo: number, minVotes: number): ControversyResponse {
...
export function buildExtremeBars(yearFrom: number, yearTo: number, minVotes: number): ExtremeBarsResponse {
...
export function buildStaffPersons(): StaffPersonsResponse {
  const persons = [
...
export function buildUserCollections(usernameOrId: string): UserCollectionsResponse {
