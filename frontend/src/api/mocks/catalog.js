...
export function workReleaseDate(w) {
    return releaseDate(w);
}
export function sumVotes(counts) {
    return counts.reduce((a, b) => a + b, 0);
}
export function computeMean(counts) {
...
export function computeMedian(counts, total) {
    if (total <= 0)
        return 0;
    const mid = (total + 1) / 2;
    let acc = 0;
...
export function computeMode(counts) {
    let best = 1;
    let bestC = -1;
    for (let i = 0; i < counts.length; i++) {
        if (counts[i] > bestC) {
...
export function computeStdDev(counts, total) {
    if (total <= 0)
        return 0;
...
export function computeExtremeRatio(counts, total) {
    if (total <= 0)
        return 0;
