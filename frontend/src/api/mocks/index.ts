...
function num(params: ParamMap, key: string, fallback: number): number {
  const v = params?.[key];
...
function str(params: ParamMap, key: string, fallback: string): string {
  const v = params?.[key];
 ...
 */
export function resolveMock(path: string, params?: ParamMap): unknown | undefined {
